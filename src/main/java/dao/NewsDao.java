...
public class NewsDao {
	public Integer add(News news, DatabaseDao databaseDao) throws SQLException {
		String sql = "insert into news(caption,author,newsType,content,newsTime,checked,publisherId) values(" + "'"
				+ "(待审核)"+news.getCaption() + "','" + news.getAuthor() + "','" + news.getNewsType() + "','" + news.getContent()
	...
	public Integer passCheck(News news, DatabaseDao databaseDao) throws SQLException {
		String sql = "update news set caption="+news.getCaption().toString()+" where newsId=" + news.getNewsId().toString();
		databaseDao.update(sql);
		sql = "update news set checked=1 where newsId=" + news.getNewsId().toString();
		return databaseDao.update(sql);
	}
