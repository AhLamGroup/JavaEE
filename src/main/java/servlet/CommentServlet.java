...
public class CommentServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (type.equals("showCommnet")) {
			...
			getServletContext().getRequestDispatcher(url).forward(request, response);
		} else if (type.equals("reviseCommnet")) {// 修改评论
			Comment comment = new Comment();
			comment.setContent(request.getParameter("content"));
			String commentId = request.getParameter("commentId");
			comment.setCommentId(Integer.parseInt(commentId));
			...
			String url = "/servlet/NewsServlet?type1=showANews&newsId=" + newsId + "&page=" + page;
			getServletContext().getRequestDispatcher(url).forward(request, response);
		} else if (type.equals("deleteCommnet")) {// 删除评论
			Comment comment = new Comment();
			...
			String url = "/servlet/NewsServlet?type1=showANews&newsId=" + newsId + "&page" + request.getParameter("page")
